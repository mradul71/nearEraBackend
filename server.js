...
//function calculating radial distance
    ...
    async function nearMe (obj, userLat, userLong){
        if(calcCrow(obj.latitude, obj.longitude, userLat, userLong) < 260){
            return obj;
        }
    }
//----------------------//
...
async function getter(id){
    var obj;
    await userRef.orderByChild("id").equalTo(id).on("child_added", (snap) => {
        obj = snap.val();
    });
...
//             // try {
//             //     const mheyark = await axios.get('http://apis.mapmyindia.com/advancedmaps/v1/9d7eb743d7c26f34ae0e02d9d5548463/geo_code', {}, {})
//             //     console.log(mark);
//             // } catch (error) {
//             //     console.log(error);
//             // }
